...
top = "/gs/bs/tga-KitaoLab-NexusYtec/yilan/projects/SAM/SAM/test_trial/pacs/post_analysis/dissociation_direction/ref_target.pdb"
base = Path("/gs/bs/tga-KitaoLab-NexusYtec/yilan/projects/SAM/SAM/test_trial/pacs/")

outdir = Path("chimerax")
outdir.mkdir(exist_ok=True)

lig_sel = "resname sam SAM"
rna_sel = "not (resname sam SAM)"
...
for trial in range(1, 21):
    ...
    coms = np.array(coms)
    np.savetxt(
        outdir / f"SAM_trial{trial:03d}.csv",
        coms,
        delimiter=",",
        header="x,y,z",
        comments=""
    )
    print(f"written SAM_trial{trial:03d}.csv")
...
for trial in range(1,21):
    print(trial)
    csv_file=f"chimerax/SAM_trial{trial:03d}.csv"
    ...
    with open(f"chimerax/SAM_trial{trial:03d}.bild","w") as f:
    ...
    print(f"written SAM_trial{trial:03d}.bild")
