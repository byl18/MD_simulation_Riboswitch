...
BASE_DIR = "/gs/bs/tga-KitaoLab-NexusYtec/yilan/projects/SAM/SAM/test_trial/pacs"
...
def worker(trial):
    ...
    xtc = md.load(f"/gs/bs/tga-KitaoLab-NexusYtec/yilan/projects/SAM/SAM/test_trial/pacs/prd_xtc/trial{str(trial+1).zfill(3)}/prd.target.trjcat-all.xtc", top=gleap_target)
    anchor_atoms = [gleap_target.top.atom(i) for i in atom_indices_rna]
    xtc.image_molecules(inplace=True, anchor_molecules=[anchor_atoms])
    ...
    plt.title(trial)
    plt.legend()
    plt.savefig('/gs/bs/tga-KitaoLab-NexusYtec/yilan/projects/SAM/SAM/test_trial/pacs/post_analysis/figures/' + str(trial+1) + '.png',dpi=200)
    plt.close()
