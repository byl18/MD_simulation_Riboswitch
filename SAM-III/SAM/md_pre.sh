######## 1 ########
mkdir 1_modeling
grep -E '^(ATOM|HETATM|TER)' 1_3e5c.pdb > 2_3e5c.pdb # delete HOH ions

# Avogadro add H & Extensions--Optimize Geometry

antechamber -fi mol2 -i 2_3e5c_Q_SAM_opt.mol2 -fo gcrt -o sam.gau -nc 0
module load gaussian
g16 < sam.gau > sam.out # Pos +1 Neg -1
antechamber -fi gout -i sam.out -fo ac -o sam.ac -c resp


antechamber -fi ac -i sam.ac -fo ac -o sam-renamed.ac -fa mol2 -a 2_3e5c_Q_SAM_opt.mol2 -ao name -at gaff2 -rn sam
antechamber -fi ac -i sam-renamed.ac -fo mol2 -o sam.mol2
parmchk2 -f mol2 -i sam.mol2 -o sam.frcmod
...
cd 2_md-preparation
pdb4amber -i ../1_modeling/2_3e5c.pdb -o sys.pdb

# delete 5'P to generate sys_no5p.pdb
tleap
...
source leaprc.gaff2
loadAmberParams ../1_modeling/sam.frcmod
sam = loadmol2 ../1_modeling/sam.mol2
saveamberparm sam sys.inpcrd sys.prmtop
saveoff sam ../sam.lib
...
solvatebox sys OPCBOX {49.22 44.35 57.03}
