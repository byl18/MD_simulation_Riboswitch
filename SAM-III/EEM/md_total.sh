...
cd 6_cmd-1
# cp ../cmd.mdp .
# gmx_mpi grompp -f cmd.mdp -c ../5_npt-10/npt.gro -t ../5_npt-10/npt.cpt -p ../2_md-preparation/gleap.top -n ../2_md-preparation/gleap.ndx -o cmd.tpr -maxwarn 2
# gmx_mpi dump -s cmd.tpr > cmd.1.txt

for cnt in {1..5}; do
if [ ! -f cmd$cnt.tpr ]; then
	if [ $cnt -eq 1 ]; then
		gmx_mpi grompp -f cmd.mdp -c ../5_npt-10/npt.gro -t ../5_npt-10/npt.cpt -p ../2_md-preparation/gleap.top -n ../2_md-preparation/gleap.ndx -o cmd$cnt.tpr -maxwarn 2
	else
		gmx_mpi grompp -f cmd.mdp -c cmd$(($cnt-1)).gro -t cmd$(($cnt-1)).cpt -p ../2_md-preparation/gleap.top -n ../2_md-preparation/gleap.ndx -o cmd$cnt.tpr -maxwarn 2
	fi
fi
...
if [ ! -f cmd$cnt.skip10.whole.target.xtc ]; then
